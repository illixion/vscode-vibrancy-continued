...
* Core:
  ...
  * Fix three ways `workbench.colorCustomizations` could be left in an unexpected state. Turning `vscode_vibrancy.disableColorCustomizations` on handed your colours back and dropped the backup, but turning it off again never re-took one — so the next disable deleted your custom background colours instead of restoring them. The same happened when the first install ran with the setting already on. And if the setting was turned on after Vibrancy had already written (a suppressed settings handler, a crash, or a hand-edited `settings.json`), disabling Vibrancy skipped the cleanup entirely, stranding translucent colours in `settings.json` and then wiping the backup that could have undone them. Vibrancy now re-takes the backup before overwriting, always cleans up writes it still owns, and only drops a backup once the restore has actually landed
  * Be much more careful about whose colours are whose. Vibrancy now records what it actually wrote and re-checks it against `settings.json` on every run, instead of trusting a backup taken once at install. Previously, if `settings.json` held a background colour Vibrancy had never captured — you edited it by hand while Vibrancy was active, or Settings Sync brought a different value from another machine — Vibrancy overwrote it with nothing recorded to put back, and disabling later deleted it instead of restoring it
  * Vibrancy now knows which VSCode profile enabled it. The effect comes from patching VSCode's own files, which are shared by every profile, but `workbench.colorCustomizations` and the record of your original colours are stored per profile. Disabling from a profile that didn't enable Vibrancy would therefore revert the shared files for everyone while only being able to restore that one profile's colours, leaving translucent values behind with no vibrancy under them — a stale transparent `terminal.background` is what put an outline around every glyph in the terminal in issue [#183](https://github.com/illixion/vscode-vibrancy-continued/issues/183). Disable now stops and explains when it's run from a profile that doesn't own the install, with a "Disable anyway" override so a deleted owner can never leave you stuck (it's also how you clear colours stranded in the current profile). Enabling from a second profile warns that it's taking ownership
  * `settings.json` is now edited with a real JSONC parser instead of regular expressions. Vibrancy normally goes through VSCode's own settings API, but the uninstall hook runs as a bare script with no extension host around it, so it used to fall back to pattern-matching the raw file — which cannot tell a colour key inside `workbench.colorCustomizations` from the same name anywhere else in the document, could not see that a match was inside a comment (a commented-out `"sideBar.background": "#282c34ff"` was mangled, previously documented as an accepted limitation), and left `"key": "value",` indentation debris behind on every restore. It now uses [`jsonc-parser`](https://github.com/microsoft/node-jsonc-parser), the same library VSCode itself uses, with edits applied to the exact ranges a key occupies so comments next to a removed setting survive. A `settings.json` that doesn't parse is now left untouched and reported, rather than being edited into something worse
  * Fix Vibrancy recording the wrong `settings.json` when enabled from a VSCode profile. The path handed to the uninstall hook was always the *default* profile's file, so uninstalling from any other profile stripped colour keys out of a file that may never have had them while leaving the real ones in place. The current profile's settings file is now resolved from VSCode's own profile registry
  * Windows now reverts `settings.json` through the same code as macOS and Linux. VSCode holds the file in memory and writes it back on exit, so the cleanup has to be deferred until after that — previously by a PowerShell script full of hand-written regexes, the second copy of a key list that had already drifted. PowerShell now only does the waiting, and the edit itself runs the same parser-based restore, staged into a temporary directory beforehand since this extension's own directory is deleted by then
  * Fix a profile created with VSCode's "Copy from profile" option adopting Vibrancy's colours as if you had chosen them. That option copies `settings.json` but not extension state, so the new profile starts out holding all 28 of Vibrancy's translucent colours with no record of what they replaced. Vibrancy identified its own writes by matching their RGB against the active theme background, which is exactly the check that fails here — the colours came from the profile that was copied. Pick a different colour theme in the new profile and every one of them was recorded as your original, so disabling Vibrancy *restored* fully transparent backgrounds with no vibrancy behind them. It now recognises its own output by its translucency rather than by which theme produced it, so a copied profile is cleaned up correctly whatever theme it uses
  * Tell profile users about the scope mismatch, once, instead of leaving them to discover it. Vibrancy's effect is machine-wide while the colours it needs are per profile, which nothing in VSCode's UI suggests — so a second profile shows the effect without the colours and looks broken. Vibrancy now explains this when it first notices more than one profile in play, and if another profile is found still holding Vibrancy's colours it names that profile and says where to disable it. When that profile doesn't have Vibrancy at all — "Copy from profile" without copying extensions leaves the colours behind with nothing there to remove them — it says that instead, rather than sending you to look for a Disable command that isn't there
  * Disabling Vibrancy no longer hands your colour customizations back before it knows it can unpatch VSCode. The restore ran first, so anything that stopped the file work afterwards — declining the administrator prompt, a write that failed or was blocked, declining the elevated retry — left VSCode still patched with the vibrancy colours already gone: every surface translucent over nothing, and no hint that running Disable again would put it right. The colours now go back only once the files really have been unpatched, so the two halves of Disable succeed or fail together
  * The extension's main entry point can now be tested. It is one large `activate()` closure that could not even be loaded outside VSCode, so the install and uninstall flows — where the file work and the settings work have to succeed or fail together — had no coverage at all; the ordering bug above is exactly the kind that hides there. Both flows now run against a throwaway VSCode install in the test suite, including the failure path
  * Which files Vibrancy patches, and which of its two runtime flavours they need, is now decided in one place with tests of its own. VSCode has moved those files three times and the layout is worked out by looking for them rather than from the version number, so a wrong answer installs a runtime the workbench cannot load and says nothing until the next start. The NixOS shadow install made that sharper: it redirects patching to a mirror that does not exist yet when the looking happens, and the code that moved the paths onto the mirror was a second, untested copy that rewrote them in place — so re-mirroring after a `nixos-rebuild` would have moved already-moved paths. Moving paths now refuses any that did not come from the directory being moved from, which turns that whole class of mistake into an error instead of a quietly mispatched editor
  * The uninstall hook no longer keeps its own copies of the colour-key list. It had two — one for macOS/Linux and one baked into the Windows deferred cleanup script — on the assumption that the hook can't import shared code, which was never true. They had drifted: `terminalStickyScroll.background` was never cleaned out of `settings.json` on Windows. Both now derive from the same list the installer uses, extended with whatever the recorded backup covers, so keys a theme introduces are cleaned up too
* Themes:
